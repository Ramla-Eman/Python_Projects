...
def calcultae_cows_and_bulls(secret,guess):
...
def main():
    ...
    print('I have generate a 4-digits number with unique digits. Try to guess it!')

    while True:
        guess = input('Guess: ')
        if len(guess) == 4 and guess.isdigit() and len(set(guess)) == 4:
            cows, bulls = calcultae_cows_and_bulls(secret,guess) 
